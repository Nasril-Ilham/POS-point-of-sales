    ...
    <div class="card-body">
        <form method="POST" action="{{ url('stok') }}" class="form-horizontal">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="number" class="form-control @error('stok_jumlah') is-invalid @enderror" id="stok_jumlah" name="stok_jumlah" value="{{ old('stok_jumlah') }}" placeholder="Masukkan jumlah stok" min="1">
                    @error('stok_jumlah')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
